...
export const useUrlParams = () => {
	...
	useEffect(() => {
		const parseUrlParams = () => {
			...
			if (Object.keys(params).length > 0 && !cookies.linkParams) {
				// Save the URL in the decoded format
				const decodedUrlParams = decodeURIComponent(JSON.stringify(params));
				setCookie('linkParams', decodedUrlParams, { path: '/' });
			}
		};
